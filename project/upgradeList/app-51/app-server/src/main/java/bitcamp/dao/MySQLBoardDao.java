...
public class MySQLBoardDao implements BoardDao {
  ...
  public List<Board> list() {
    try (PreparedStatement stmt = ds.getConnection(false)
        .prepareStatement("select" + " b.board_no," + " b.title," + " b.view_count,"
            + " b.created_date," + " s.soldier_no," + " s.name"
            + " from myapp_board b inner join myapp_soldier s on b.writer=s.soldier_no"
      ...
      try (ResultSet rs = stmt.executeQuery()) {
        ...
        while (rs.next()) {
          ...
          writer.setNo(rs.getInt("soldier_no"));
  ...
  public Board findBy(int no) {
    try (PreparedStatement stmt = ds.getConnection(false)
        .prepareStatement("select" + " b.board_no," + " b.title," + " b.content," + " b.view_count,"
            + " b.created_date," + " s.soldier_no," + " s.name"
            + " from myapp_board b inner join myapp_soldier s on b.writer=s.soldier_no"
  ...
  public int update(Board board) {
    try (PreparedStatement stmt = ds.getConnection(false).prepareStatement("update myapp_board set"
        + " title=?," + " content=?" + " where category=? and board_no=? and password=sha1(?)")) {
