...
public class MySQLSoldierDao implements SoldierDao {
  ...
  public MySQLSoldierDao(DataSource ds) {
    this.ds = ds;
  }
  ...
  public void insert(Soldier soldier) {
    try (PreparedStatement stmt = ds.getConnection(false)
        .prepareStatement("insert into myapp_soldier"
  ...
  public List<Soldier> list() {
    try (PreparedStatement stmt = ds.getConnection(false)
        .prepareStatement("select soldier_no, soldier_milnum, name, age, `rank`,"
  ...
  @Override
  public Soldier findByMilnumAndPassword(Soldier param) {
    try (PreparedStatement stmt = ds.getConnection(false)
        .prepareStatement("select soldier_no," + " soldier_milnum," + " name, age, `rank`,"
            + " enlis_date, dis_date, d_day" + " from myapp_soldier"
            + " where soldier_milnum=? and password=sha1(?)")) {

      stmt.setString(1, param.getMilitaryNumber());
  ...
  public int update(Soldier soldier) {
    try (Statement stmt = ds.getConnection(false).createStatement()) {
  ...
  public int delete(int no) {
    try (PreparedStatement stmt =
        ds.getConnection(false).prepareStatement("delete from myapp_soldier where soldier_no=?")) {
